...
UNITS_REGEX = re.compile(r'.*\[([^/]*)/?(.*)\]', flags=RE_FLAGS)
...
def blood_panel_replacements(desc):
    ...
    for phrase, replacement in replacements.items():
        desc = re.sub(phrase, replacement, desc, flags=RE_FLAGS)
    return desc


def units_replacements(desc):
    # Strip out units in square brackets
    match = UNITS_REGEX.match(desc)
    if match is None:
        return desc
    ...
    replacement = UNITS_REPLACEMENT_HEAD.get(head, head)
    if tail:
        tail = UNITS_REPLACEMENT_TAIL.get(tail, tail)
        replacement = f'{replacement}/{tail}'

    beginning = desc[:match.span(1)[0]]
    end = desc[match.span(2)[1]:]
    return ''.join((beginning, replacement, end))
...
class PhenotypePlotter:
    ...
    def get_stripped_label_description(self, name, width):
        ...
        # Strip out units in square brackets
        desc = units_replacements(desc)
        # Strip out "in Blood" etc
        desc = blood_panel_replacements(desc)
